...
RSpec.describe CheckoutService do
  describe '#checkout' do
    ...
    let(:coffee) { Product.create!(code: 'CF1', name: 'Coffee', price: 11.23, bulk_eligible: true, bulk_threshold: 3, discount_factor: 0.666) }

    before do
      # Prepare basket items
      basket.basket_items.create!(product: green_tea, quantity: 2, discount_price: 3.11)  # BOGO will make one free
      basket.basket_items.create!(product: strawberries, quantity: 3, discount_price: 4.50)  # Price drops to 4.50 each
      basket.basket_items.create!(product: coffee, quantity: 2, discount_price: 11.23)  # Price doesn't change
    ...
    it 'correctly calculates the total and prepares itemized receipt with discount messages' do
      ...
      result = checkout_service.perform_checkout

      expect(result[:total]).to eq(3.11 + (4.50 * 3) + (11.23 * 2))
